...
export class ServicioArchivos{
    ...
    async guardarArchivo(fichero: Fichero, ruta: string, idUsuario: string): Promise<ArchivoGuardado>{
        try {
            ...
            return await this.http.post<ArchivoGuardado>(`${this.host}${endpoint}`, formData, {  Authorization: `Bearer d4a32a3b-def6-4cc2-8f77-904a67360b53` })
        } catch (error) {
            console.log(error);
            throw error
        }
